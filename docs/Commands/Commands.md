...
Show a list of the jobs that were previously run in the local folder including the bpipe command line and arguments.## The jobs command
...
The main feature of 'retry' is that it automatically looks up the exact command that you ran the previous time you ran Bpipe in the local directory and re-executes it with the same options and input files. ## The run command
...
Displays a list of currently running commands by the pipeline in the current directory.## The stop command
...
Stop the current job (if any) that is currently running in the local folder.  Any inputs not finished being created by the currently executing pipeline stage(s) will be cleaned up and moved to the [[Trash|Trash Folder]].## The test command
